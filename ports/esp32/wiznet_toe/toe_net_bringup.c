...
void toe_net_shutdown(void) {
    ...
    #if MICROPY_WIZNET_TOE_SOCKET_WRAP
    // Stop claiming new sockets before tearing the chip down, so a socket()
    // racing with this does not get a hardware socket we are about to kill.
    toe_vfs_set_claims_new_sockets(false);
...
bool toe_net_bringup(const wiz_NetInfo *net_info) {
    ...
    #if MICROPY_WIZNET_TOE_SOCKET_WRAP
    // The reset above closed every hardware socket, so the wrap layer's
    ...
    // From here on socket() hands out TOE hardware sockets. Before this point
    // (and in a firmware that never calls net_init) socket() is untouched.
    toe_vfs_set_claims_new_sockets(true);
    #endif
