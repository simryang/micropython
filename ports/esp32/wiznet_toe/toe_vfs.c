...
static bool s_claims_new_sockets;
...
bool toe_vfs_claims_new_sockets(void) {
    return s_claims_new_sockets;
}

void toe_vfs_set_claims_new_sockets(bool claims) {
    s_claims_new_sockets = claims;
}
...
static int toe_vfs_close(int sn) {
    ...
    // __wrap_lwip_close), so a socket reaching here is genuinely going away.
