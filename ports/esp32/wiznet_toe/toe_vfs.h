...
//   2. TOE fds can no longer collide with lwIP socket fds, so the __wrap_lwip_*
//      layer can dispatch by fd and fall through to __real_lwip_* for
...
// wiztoe_wrap.c __wrap_lwip_close -- because a permanent=false registration
// can only be freed by close(), not esp_vfs_unregister_fd.)
void toe_vfs_release_all(void);

// True once net_init() has brought the chip up, i.e. new socket() calls
// should be served by the TOE rather than by lwIP.
bool toe_vfs_claims_new_sockets(void);
void toe_vfs_set_claims_new_sockets(bool claims);
